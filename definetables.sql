...
-- insert a periode with name thisyear/nextyear => 2012/2013 etc    
...
CREATE TABLE quiz_useranswer (
    ...
    cid int references quiz_question on delete cascade,
    userid int references users on delete cascade,
    response varchar,
    ...
    time int,                      -- timestamp for last grading
    UNIQUE (userid,qid,cid),
    feedback varchar default '',   -- feedback after question is answered, dynamic elements
    hintcount smallint default 0,  -- how many hints used
    firstseen int default 0,       -- timestamp for first view
