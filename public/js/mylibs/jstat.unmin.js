...
jstat.plot = function(target,x, y, options) {
    if(x == null) {
    ...
    $j('#' + hash).dialog({
        ...
        resizable: true,
        resize: function() {
            $j.plot($j('#graph-' + hash), [series], flotOpt);
        },
...
var Plot = Class.extend({
    ...
    render: function() {
        this._flotObj = null;
        this._flotObj = $j.plot($j(this._container), this._plots, this._options);
    }
});
    ...
    setHover: function(bool) {
        if(bool) {
            ...
            function showTooltip(x, y, contents, color) {
                $j('<div id="jstat_tooltip">' + contents + '</div>').css( {
                    position: 'absolute',
                    display: 'none',
                    top: y + 15,
                    'font-size': 'small',
            ...
            $j(this._container).bind("plothover", function(event, pos, item) {
                $j("#x").text(pos.x.toFixed(2));
                $j("#y").text(pos.y.toFixed(2));
                if (item) {
                    if (previousPoint != item.datapoint) {
                        previousPoint = item.datapoint;
                        $j("#jstat_tooltip").remove();
            ...
            $j(this._container).bind("mouseleave", function() {
                if($j('#jstat_tooltip').is(':visible')) {
                    $j('#jstat_tooltip').remove();
                    previousPoint = null;
                }
            });
            ...
            $j(this._container).unbind("plothover");
        }
