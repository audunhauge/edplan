...
j = jstat;
/*
 * Simplified for use with node.js
 * Dropped all graphing support and jquery
 * 
*/
...
jstat.dnorm = function(x, mean, sd, log) {
    ...
    if(!isNaN(x)) {
        ...
        console.log( "Illegal argument: x");
        return [];
    }
}
...
jstat.bd0 = function(x, np) {
    var ej, s, s1, v, j;
    if(!jstat.isFinite(x) || !jstat.isFinite(np) || np == 0.0) {
      console.log( "illegal parameter in jstat.bd0");
      return 0;
    }
...
jstat.lgamma = function(x) {
    function lgammafn_sign(x, sgn) {
        ...
        if(Math.abs((x-jstat.trunc(x-0.5))* ans / x) < dxrel) {
            console.log( "The answer is less than half the precision argument too close to a negative integer");
            return 0;
        }
        return ans;
    }
...
jstat.lgammacor = function(x) {
    ...
    if(x < 10) {
        ...
        console.log( "Underflow error in lgammacor");
        return 0;
    } else if (x < xbig) {
        tmp = 10 / x;
...
jstat.incompleteBeta = function(a, b, x) {
    ...
    if(x < 0.0 || x > 1.0) {
        console.log( "bad x in routine incompleteBeta");
        return 0;
    }
    if(x == 0.0 || x == 1.0) {
        bt = 0.0;
...
var ContinuousDistribution = Class.extend({
    ...
    density: function(valueOrRange) {
        if(!isNaN(valueOrRange)) {
            ...
            console.log( "Invalid parameter supplied to " + this.getClassName() + ".density()");
            return [];
        }
    },
    cumulativeDensity: function(valueOrRange) {
        if(!isNaN(valueOrRange)) {
            ...
            // neither value or range
            console.log( "Invalid parameter supplied to " + this.getClassName() + ".cumulativeDensity()");
            return [];
        }
    },
...
var NormalDistribution = ContinuousDistribution.extend({
    ...
    _pdf: function(x, give_log) {
        ...
        if(sigma<=0) {
            if(sigma < 0) {
                console.log( "invalid sigma in _pdf");
                return Number.NaN;
            }
...
var LogNormalDistribution = ContinuousDistribution.extend({
    ...
    _pdf: function(x, give_log) {
        ...
        if(sdlog <= 0) {
          console.log( "Illegal parameter in _pdf");
          return Number.NEGATIVE_INFINITY
        }
    ...
    _cdf: function(x, lower_tail, log_p) {
        ...
        if(sdlog <= 0) {
            console.log( "illegal std in _cdf");
            return Number.NEGATIVE_INFINITY
        }
...
var GammaDistribution = ContinuousDistribution.extend({
    ...
    _pdf: function(x, give_log) {
        ...
        if(shape < 0 || scale <= 0) {
            console.log( "Illegal argument in _pdf");
            return Number.NEGATIVE_INFINITY
        }
     ...
     since pt(q, df, ncp) <= pt(0, df, ncp) = \Phi(-ncp) */
      ...
      FIXME: test should depend on `df', `tt' AND `del' ! */
        ...
        if (x > 0.) {/* <==>  t != 0 */
            ...
            if(it == ITRMAX) {
                console.log( "Non-convergence _pnt");
                return Number.NEGATIVE_INFINITY
            }
        } else {
            tnc = 0.;
