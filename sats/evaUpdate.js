/*
 * Oppdater med ny sats fil fra eva
...
function pro(client) {
fs.readFile('eva_utf8.txt', 'utf8',function (err, data) {
  if (err) throw err;
  ...
  while (i < l) {
    ...
    if ( line.substr(0,12) == 'Group (6201)' ) {
      ...
      for (var gr in groupmem) {
              if (groupmem[gr] == '' || groupmem[gr] == "\r") continue;
            ...
            if (tid) {
                ...
                do {
                    if (strict && timetable[tti][day][mislot]) {
                        ...
                        ttlist.push( "(2456153,"+tid+","+rid+","+mycid+",'timetable',"+day+","+mislot+",'"+subj_group+"','"+room+"')" );
                    }
                    midura -= 40;
                    mislot++;
                } while (midura > 0);
